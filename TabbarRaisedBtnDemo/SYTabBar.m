...
- (UIButton *)btn{
    ...
    if (_btn==nil) {
        ...
        [_btn setImage:[UIImage imageNamed:@"roundbutton"] forState:UIControlStateNormal];
        //button的大小与图片一致
        [_btn sizeToFit];
...
- (void)layoutSubviews{
    ...
    //设置自定义button的位置
    self.btn.frame = CGRectMake(width * 2 + (width - height) / 2, -height / 2, height, height);
...
- (void)resetPlusBtnStyle {
    [self.btn setImage:[UIImage imageNamed:@"roundbutton"] forState:UIControlStateNormal];
